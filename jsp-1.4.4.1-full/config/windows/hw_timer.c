...
void hw_timer_initialize()
{
	...
	FatalAssertion(result == TIMERR_NOERROR &&  TIMER_CLOCK_WINDOWS >= devcap.wPeriodMin && TIMER_CLOCK_WINDOWS <= devcap.wPeriodMax, "指定されたクロックティックはマルチメディアタイマで設定可能な範囲を超えています。");

	timeBeginPeriod(TIMER_CLOCK_WINDOWS);	//timeBeginPeriod(devcap.wPeriodMin); と実行すると、負荷と引き換えに精度が上がる
