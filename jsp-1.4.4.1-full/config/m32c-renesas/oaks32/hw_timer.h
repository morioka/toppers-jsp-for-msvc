 ...
 * 
 *  @(#) $Id: hw_timer.h,v 1.4 2005/12/06 02:55:09 honda Exp $
 */
 ...
 */
#define TIMER_CLOCK		(30000u)	/* 30MHz */
...
hw_timer_initialize(void)
{
	...
	/* タイマカウント停止  */
	tabsr = sil_reb_mem((VP)TADR_SFR_TABSR);
	tabsr &= ~TBIT_TABSR_TA0S;
	sil_wrb_mem((VP)TADR_SFR_TABSR, tabsr);

	sil_wrb_mem((VP)TADR_SFR_UDF, 0x00);	/* ダウンカウント設定	*/
	sil_wrb_mem((VP)TADR_SFR_TA0MR, 0x00);	/*  カウントソースf1, タイマモード  */
	/*
	 *  タイマは 1msで TIMER_CLOCK カウント,
	 *  レジスタ設定値はマイナス1する.
	 */
	sil_wrh_mem((VP)TADR_SFR_TA0, TO_CLOCK(TIC_NUME, TIC_DENO)-1);/* タイマ値の初期化 */
...
{
	return ((CLOCK)(TO_CLOCK(TIC_NUME, TIC_DENO)-1)-sil_reh_mem((VP)TADR_SFR_TA0));
}
