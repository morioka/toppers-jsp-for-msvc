 ...
 *
 *  その後, スタックポインタの値を r3r1 に, 
 *  割込みハンドラのアドレスを a0 に格納した状態で
 *  割込み出入口処理の後半へジャンプする.
 */
 ...
 *  タックポインタの値をR3R1レジスタにセットし、アセンブラで記述された
 *  ハンドリングプログラムに分岐する．
 */
...
asm("	stc	isp, r3r1		; スタックポインタを取り出し	");\
asm("	mov.l	#_" #inthdr ", a0	; 割込みハンドラのアドレスをa0へ");\
...
exc_sense_lock(VP p_excinf)
{
	 ...
	 *
	 *  下式の定数28はCPU例外入り口処理でレジスタがスタックにどう
	 *  保存されているかに依存して決定する数値
	 */
