 ...
 *
 *  @(#) $Id: tool_config.h,v 1.6 2005/11/13 14:05:01 honda Exp $
 */
 ...
 */
#define __EMPTY_LABEL(x,y) x y[1]
...
/*
 *  GNU ldのPROVIDE命令と同等の機能がないため、#ifdefで処理している。
 *  外部定義の処理があれば、関数名をSOFTWARE_TERM_HOOKマクロとして
 *  定義する。sys_config.hで定義するのを標準とする。
 *  　　・外部定義があれば、それを呼び出す
 *  　　・外部定義がなければ、何もしない（空の定義）
 */
#ifdef SOFTWARE_TERM_HOOK
extern void SOFTWARE_TERM_HOOK(void);
#define call_atexit SOFTWARE_TERM_HOOK

#else	/*  SOFTWARE_TERM_HOOK  */
#define call_atexit()

#endif	/*  SOFTWARE_TERM_HOOK  */
